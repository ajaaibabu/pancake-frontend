...
import { usephoenix, useBunnyFactory } from 'hooks/useContract'
...
const minimumphoenixBalanceToMint = new BigNumber(MINT_COST).multipliedBy(DEFAULT_TOKEN_DECIMAL)

const Mint: React.FC = () => {
  const [variationId, setVariationId] = useState<Nft['variationId']>(null)
  const { actions, minimumphoenixRequired, allowance } = useProfileCreation()

  const { account } = useWeb3React()
  const phoenixContract = usephoenix()
  const bunnyFactoryContract = useBunnyFactory()
  const { t } = useTranslation()
  const hasMinimumphoenixRequired = useHasphoenixBalance(minimumphoenixBalanceToMint)
  const { isApproving, isApproved, isConfirmed, isConfirming, handleApprove, handleConfirm } =
    useApproveConfirmTransaction({
      onRequiresApproval: async () => {
        // TODO: Move this to a helper, this check will be probably be used many times
        try {
          const response = await phoenixContract.allowance(account, bunnyFactoryContract.address)
      ...
      onApprove: () => {
        return phoenixContract.approve(bunnyFactoryContract.address, allowance.toJSON())
      },
      onConfirm: () => {
  ...
  return (
    <>
      ...
      <Text as="p" mb="24px">
        {t('You can change your profile pic later if you get another approved Panphoenix Collectible.')}
      </Text>
      <Card mb="24px">
        <CardBody>
          ...
          <Text as="p" mb="24px" color="textSubtle">
            {t('Cost: %num% phoenix', { num: MINT_COST })}
          </Text>
          {nfts.map((nft) => {
            ...
            return (
              <SelectionCard
                ...
                disabled={isApproving || isConfirming || isConfirmed || !hasMinimumphoenixRequired}
              >
          ...
          {!hasMinimumphoenixRequired && (
            <Text color="failure" mb="16px">
              {t('A minimum of %num% phoenix is required', { num: MINT_COST })}
            </Text>
          )}
          <ApproveConfirmButtons
            isApproveDisabled={variationId === null || isConfirmed || isConfirming || isApproved}
            isApproving={isApproving}
            isConfirmDisabled={!isApproved || isConfirmed || !hasMinimumphoenixRequired}
